...
T CursorList<T>::pop_front(){
    ...
    count--;
    if(count == 0) //jesli lista jest pusta to head ma byc w tym samym miejscu w tablicy co spare
        head = spare;
...
T CursorList<T>::erase(int i){ //deletes and returns i-th element
    ...
    for(int n = 0; n < i - 1; ++n){
        temp = array[temp].next;
    } //temp przechowuje indeks (i-1)-go elementu
    int prev_spare = spare; //zapisuje indeks spare sprzed usunięcia elementu
